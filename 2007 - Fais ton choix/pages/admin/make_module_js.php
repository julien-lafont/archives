...
// ------------------------------------------------------------------------------------ //
//              Met à jour le fichier module.js permettant à un webmaster               //
...
$c='<div id="inner_faistonchoix" style="width:520px; border:1px solid #ccc; padding:5px; text-align:center">
		<a href="'.URL.'rss.xml" target="_blank" title="Ajouter le flux rss du site à mon agrégateur favoris" /><img src="'.URL.'images/rss1.png" border="0" style="float:right;" alt="Rss"/></a>
	...
	<div id="sous_titre" style="font-family:Verdana; text-align:center; padding-left:15px; color:#777; font-size:13px; font-variant:small-caps; margin-left:20px">et décide qui sera le vainqueur</div>
	...
	<span style="font-family:Helvetica, sans-serif; color:#0066FF; font-size:11px">Vote pour l\'opposant que tu veux défendre en cliquant sur sa photo.</span>
	
</div>';

// On insère dan le code jvs ( en compressant le tout )
$code="try {
	div = document.getElementById('faistonchoix');
	div.innerHTML = '".addslashes(preg_replace ('/('.CHR(9).'|'.CHR(13).'|'.CHR(10).')/', "", $c))."';
} 
catch (e) { alert('Vous devez créer un div ayant pour id \'faistonchoix\' pour que le script fonctionne'); throw e }";
