...
// Page principale : affiche les commandes finies (expédiées depuis plus d'une semaine) :
if (empty($id)) {

	$design->zone("titre", "Historique des mes commandes");
	
	$c='<div class="centre">Cette page vous permet de garder une trace de toutes les commandes passées via notre boutique en ligne.</div>';
	
	$sql=mysql_query("SELECT * FROM ".PREFIX."commandes WHERE id_membre=".$_SESSION['sess_id']." AND statut='expedie' AND envoie_date<(NOW()-3600*24*7)") or die(mysql_error());
	...
	$c='<table class="table_articles"  cellspacing="0" cellpadding="0">';
		...
		foreach($liste_produits as $idArray=>$infosArray) {
			...
			$c.='<tr '.$class.'>
					...
					<td class="b2">							
						<span style="font-weight:bold; font-size:14px; color:#000">'.$nbProduit.' '.$terme.'</span>
					</td>
					<td class="c2">
