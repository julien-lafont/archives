	 ...
	 if (pseudo!='') 
	 {
		if (pseudo.length >= 3 && pseudo.length <= 20) {
			...
			if (arguments.length==0) {
				ajax('get','pages/verif_inscription_ajax.php','pseudo='+escape(pseudo),'verifPseudoAjax');
			} else {
				good('pseudo');
			}
...
function verifEmail()
{
   ...
   if ((arobase < 3)||(point + 2 > email.length) ||(point < arobase+3)) {
	   bad('email', 'Adresse email invalide');
	   	...
	   	// ajax ou non ?
		if (arguments.length==0) {
			ajax('get','pages/verif_inscription_ajax.php','email='+escape(email),'verifEmailAjax');
		} else {
			good('email');
		}
   }
