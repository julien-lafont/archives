	...
	switch($type)
	{
		...
		case "erreur":
			return '<div style="width:100%; text-align:center; margin-top:50px; margin-bottom:50px;">
						...
						<a href="http://www.studio-dev.fr" title="Accueil du site Studio-Dev.fr">Accueil Studio-Dev</a>
					</div>';
		break;
		default:
			return $texte;
...
function connexion($login, $pass) {
	...
	$ip = ip();
	$sql = mysql_query("SELECT id, pseudo, mdp, cle, groupe FROM ".PREFIX."membres WHERE pseudo='$login' AND groupe>=1 AND groupe!=".GROUPE_BAN) or die(mysql_error());
	$result = mysql_fetch_object($sql);
			...
			if ( $result->mdp==$pass AND $result->id!=0 ) 
			{
						...
						// Maj des données
						$sqlMaj = mysql_query('UPDATE '.PREFIX.'membres SET last_ip="'.$ip.'" WHERE pseudo="'.$login.'" ');
						return true;									
			} else {	
						return false;										
			}
