...
class IxYaml {
  ...
  */
  ...
  /*
   * Dump depuis un objet de type stdClass au format Yaml
   */
  public function dumpObjet($obj) {
    return $this->dump($this->object2array($obj));

  }

  private function object2array($objet)
  {
    if( !is_object( $objet ) )
      return $objet ;
    $tableau = array();
    foreach($objet as $cle => $valeur )
      $tableau[$cle] = $this->object2array ( $valeur ) ;
    return $tableau;
  }
...
// LOADING FUNCTIONS
  ...
  private function addArray ($array, $indent) {
    ...
    $tempPath = IxYaml::flatten ($this->path);
