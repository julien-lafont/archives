<?php
/**
 * Include (require) this page if your application needs to use the class Gmailer.
 * This library (libgmailer.php) is defines 3 classes: GMailer, GMailSnapshot
 ...
 * @var bool
 */
define("GM_USE_LIB_AS_MODULE",		false);	// false == Normal operation
...
define("GM_LNK_LOGIN_REFER",		"https://www.google.com/accounts/ServiceLogin?service=mail&passive=true&rm=false&continue=http%3A%2F%2Fmail.google.com%2Fmail%3Fui%3Dhtml%26zy%3Dl&ltmpl=default&ltmplcache=2&hl=en");
// Added by Neerav; 5 June 2005
// yj_blanco change to default; Neerav; 1 Nov 2007
...
// Updated by Neerav; 22 Jan 2008
define("GM_USER_AGENT", "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.8.1.11) Gecko/20071127 Firefox/2.0.0.11");
 ...
 * Other constants.
*/
define("GM_VER", "1.0.0 alpha 22Jan08");
 ...
 * for their bug reports, comments and advice.
 *
 * @package GMailer
...
/* 			$postdata .= "&ltmpl=yj_blanco"; */
			...
			//$postdata .= "GALX=";	// added but not used yet; Neerav; 1 Nov 2007; 
		}

		// password debugging
		//Debugger::say(basename(__FILE__).": ".__LINE__.": ".print_r($this->pwd,true));
		//Debugger::say(basename(__FILE__).": ".__LINE__.": ".print_r(urlencode($this->pwd),true));
		//Debugger::say(basename(__FILE__).": ".__LINE__.": ".print_r(urlencode(urldecode($this->pwd)),true));
		//Debugger::say(basename(__FILE__).": ".__LINE__.": ".print_r($postdata,true));
		//exit;
		...
		if ($this->domain) {
			...
			if (	strpos($this->gmail_data,'<div class="x"> Sign in to your account at <h2>') > 0
				...
				// domain exists
				///Debugger::say(basename(__FILE__).": ".__LINE__.": "."hosted domain found");
		...
		/* Let's begin the signin process */
		...
		// Gmail's "user ping", sending word that this user is about to sign in.
		// added in case it's needed in the future; Neerav; 1 Sept 2006
		// user pings are also done by non-GAFYD
		//if ($this->domain) {
		//	$this->gmail_data = GMailer::execute_curl(
		...
		//		'cookie', "$login_cookie"
		//	);
		//	Debugger::say(basename(__FILE__).": ".__LINE__.": "."user ping result: ".print_r($this->gmail_data,true));
		//}

		// double confirm the post data
		///Debugger::say(basename(__FILE__).": ".__LINE__.": post data for signin: ".print_r($postdata,true));
		//exit;
		...
		$this->gmail_data = GMailer::execute_curl(
			...
			//'nocookie', ""
			(($this->domain) ? 'nocookie': 'cookie'), (($this->domain) ? '' : "$login_cookie")
			//,false // update cookie
		);
		...
		/** from here we have to perform "cookie-handshaking"... (multiple redirects) **/		

		///Debugger::say(basename(__FILE__).": ".__LINE__.": "."current cookie array: ".print_r($this->cookie_array,true));
		$cookies = GMailer::get_cookies($this->gmail_data);
		///Debugger::say(basename(__FILE__).": ".__LINE__.": "."user/pass phase cookies: ".print_r($this->cookie_array,true));
		//exit;
		...
		if (/* $this->domain and  */	// Any account may have already returned the Auth string
			...
			$cookies = "";
			$auth_received = true;
			$second = $this->gmail_data; // skip the Auth phase
			///Debugger::say(basename(__FILE__).": ".__LINE__.": "."hosted domain auth recieved: ".print_r($this->gmail_data,true));
		...
		$redirect_url		= "";
		$redirect_count		= 0;
		$refer				= (($this->domain) ? $this->GM_LNK_LOGIN : $this->GM_LNK_LOGIN_REFER);

		while(true) {
			$redirect_count++;
		
			if ($redirect_HID and GMailer::gmail_cookie('HID') !== "") {
				$redirect_HID = false;
				$redirect_url = $this->GM_LNK_GMAIL.'?auth='.str_replace("HID=", "", GMailer::gmail_cookie('HID'));
...
/* 				$redirect_url = ""; */
			...
			} elseif (preg_match("/<meta( http-equiv=\"refresh\")? content=\"0;\s*url='?([^\"\']*)'?\"/",$this->gmail_data,$matches)) {
				$redirect_url = str_replace("&amp;","&",$matches[2]);
			}
			...
			if ($redirect_url != "") {
				...
				///Debugger::say(basename(__FILE__).": ".__LINE__.": "."Redirection $redirect_count result:\n\n".print_r($this->gmail_data,true));
				$cookies = GMailer::get_cookies($this->gmail_data);
				///Debugger::say(basename(__FILE__).": ".__LINE__.": "."cookies after Redirection $redirect_count:\n".print_r($this->cookie_array,true));
				$refer = $redirect_url;
				$redirect_url = "";
			} else {
				break;
		...
		///Debugger::say(basename(__FILE__).": ".__LINE__.": "."cookies (pre-clean): ".print_r($this->cookie_array,true));
		// remove test cookie
		if (isset($this->cookie_array['dhc'])) unset($this->cookie_array['dhc']);
		// remove unnecessary GoogleAccountsLocale_session, if it still exists.
		if (isset($this->cookie_array['GoogleAccountsLocale_session'])) unset($this->cookie_array['GoogleAccountsLocale_session']);
		
		// remove cookies outside of the /mail path, we won't need them anymore
		foreach($this->cookie_array as $name => $value) {
			if ($value['path'] == '/accounts') unset($this->cookie_array["$name"]);
		}
		///Debugger::say(basename(__FILE__).": ".__LINE__.": "."final cookies (corrected/cleaned): ".print_r($this->cookie_array,true));
	...
	*/
	function fetch($query) {
		if ($this->isConnected() == true) {
			...
			$this->gmail_data = GMailer::execute_curl(
				$this->GM_LNK_GMAIL."?ui=1&".$query,
				$this->GM_LNK_GMAIL."?ik=&ui=1&search=inbox&view=tl&start=0&init=1".$this->proxy_defeat(),
				'get'
			);
	...
	* 1. {@link GM_STANDARD}: For standard mail-boxes like Inbox, Sent Mail, All, etc. In such case, $box should be the name of the mail-box: "inbox", "all", "sent", "draft", "spam", or "trash". $parameter would be used for paged results.
...
/* 				//Debugger::say(basename(__FILE__).": ".__LINE__.": ".$query); */
/* 				Debugger::say(basename(__FILE__).": ".__LINE__.": "."Thumbnail cookie: ".$this->cookie_str); */
...
/* 					$this->GM_LNK_GMAIL_HTTP."x/".str_replace("&zx=","",$this->proxy_defeat("nodash"))."-/?mi=0&th=".urlencode($msg_id)."&v=att", */
				...
				if (strpos($this->gmail_data,'gmail_error=') !== false) {
					...
					if (	$error != 25	// 
						...
						Debugger::say(basename(__FILE__).": ".__LINE__.": ".print_r($this->gmail_data,true),"error_log.glib.as_html.php");
					}
			...
			// view all images
/* 				$query = $this->GM_LNK_GMAIL."?ui=1&view=att&disp=imgs&th=".urlencode($msg_id); */
			// view image inline
/* 				$query = $this->GM_LNK_GMAIL."?ui=1&view=att&disp=inline&attid=".urlencode($attach_id)."&th=".urlencode($msg_id); */
			...
			if ($fp) {
				...
				if ($action == "embed_head") {
					...
					//Debugger::say(basename(__FILE__).": ".__LINE__.": "."url: ".$query);
				...
				curl_setopt($c, CURLOPT_SSL_VERIFYPEER, FALSE);
				curl_setopt($c, CURLOPT_USERAGENT, GM_USER_AGENT);
				curl_setopt($c, CURLOPT_REFERER, $this->GM_LNK_GMAIL."?ui=1&ik=&search=inbox&view=tl&start=0&init=1".$this->proxy_defeat());
...
/* 			$postdata["msgbody"] 	= stripslashes($body); */
			$postdata["msgbody"] 	= $body;	// removed stripslashes; Neerav; 12 Sept 2007
			...
			if ($this->domain) {
				...
				//Debugger::say(basename(__FILE__).": ".__LINE__.": "."pre sending message: ".print_r($this->gmail_data,true));
			...
			// non-hosted domain
				...
				$this->gmail_data = GMailer::execute_curl(
					$this->GM_LNK_GMAIL."?ui=1&search=inbox&qt=&cmid=&newatt=".$new_attach."&rematt=0".$getdata,
					$this->GM_LNK_GMAIL."?ui=1&view=cv&search=inbox&th=".$tid/* ."&lvp=4&cvp=1" */."&qt=".$this->proxy_defeat("nodash"),
					'post',
					$postdata
				);
			}
...
/* 				$this->gmail_data = GMailer::execute_curl( */
/* 					$this->GM_LNK_GMAIL."?ui=1&".$link */
/* 					$this->GM_LNK_GMAIL."?ui=1&".$referrer, */
/* 					'post', */
/* 					$postdata */
/* 				); */
		if ($this->isConnected()) {			
			...
			if ($act == GM_ACT_TRASHMSG || $act == GM_ACT_UNTRASHMSG || $act == GM_ACT_DELTRASHEDMSGS) {
				$this->gmail_data = GMailer::execute_curl(
					$this->GM_LNK_GMAIL."?ui=1"."&qt=".$query."&view=up".$postdata.$this->proxy_defeat(),
					"",
					'get'
				);
			} elseif ($act == GM_ACT_EMPTYSPAM || $act == GM_ACT_EMPTYTRASH) {
				$refer = $this->GM_LNK_GMAIL."?ui=1";
				...
				$this->gmail_data = GMailer::execute_curl(
					$this->GM_LNK_GMAIL."?ui=1&".$query."&view=tl&start=0".$this->proxy_defeat()."&ba=1&act=dl".(($id != NULL and $id != "") ? "&msq=".$id: '')."&at=".$this->at_value(),
					$refer,	
					'get'
				);
			} else {
				$this->gmail_data = GMailer::execute_curl(
					$this->GM_LNK_GMAIL."?ui=1&".$query."&view=tl&start=0",
					$referrer,
					'post',
					$postdata
				);
...
/* 				header("Set-Cookie: ".GM_COOKIE_IK_KEY."=0; Discard; Domain=".$domain.";"); */
	...
	*/
	function disconnect() {
		...
		$this->gmail_data = GMailer::execute_curl(
			...
			$this->GM_LNK_GMAIL."?ui=1&logout&hl=en".$this->proxy_defeat("nodash"),
			$this->GM_LNK_GMAIL."?ui=1&ik=&search=inbox&view=tl&start=0".$this->proxy_defeat("nodash"),
			'get'
		);
		//GMailer::parse_gmail_response($this->gmail_data);
	...
	*/
	function invite($email) {
		...
		if ($this->isConnected()) {			
			...
			$this->gmail_data = GMailer::execute_curl(
				$this->GM_LNK_GMAIL."?ui=1&view=ii",
				$this->GM_LNK_INVITE_REFER."&ui=1",
				'post',
				$postdata
			);
	...
	*
	* @access private
	* @param array $name cookies name(s) (blank for all cookies)
	* @return string
	* @author Neerav
	* @since 5 Nov 2007
	*/
	function gmail_cookie($name = "", $inc_path = "") {

		//Debugger::say(basename(__FILE__).": ".__LINE__.": "."all cookies: ".print_r($this->cookie_array,true));

		// requested one cookie
		if (is_string($name) and $name !== "") {
			if ($name !== "" and isset($this->cookie_array["$name"])) {
				return $this->cookie_array["$name"]['name'].'='.$this->cookie_array["$name"]['value'];
			} else {
				return "";
			}
		// request 2+ or all cookies
		} else {
			$count_cookie = ($name === "") ? 0 : count($name);
			$cookie_string = "";
			foreach ($this->cookie_array as $cookie_name => $cookie_data) {
				if ($count_cookie and !in_array($cookie_name,"$name")) continue;
				if ($cookie_data['path'] != "" and $cookie_data['path'] != $inc_path) continue;
				if ($cookie_string != "") $cookie_string .= "; ";
				$cookie_string .= $cookie_data['name'].'='.$cookie_data['value'];
			}
	...
	* @static
	*/
	function get_cookies($header, $return = 'string') {
								...
								if ($i < $input_length - 2) { 
									switch($input[$i+2]) {
										...
										case "'":							// the case \\'
											...
											$i += 1;
											break;
										default:	// added to preserve backslashes in the message text; Neerav; 12 Sept 2007
											$dataHold .= '\\';
											$lastCharacter = '\\';
											$i += 1;
											break;
...
/* 						} */
			...
			$postdata["at"] = $this->at_value();
			//Debugger::say(basename(__FILE__).": ".__LINE__.": "."edit contact post: ".print_r($postdata,true));
			//Debugger::say(basename(__FILE__).": ".__LINE__.": "."edit contact post: ".print_r($details,true));

			$this->gmail_data = GMailer::execute_curl(
				$this->GM_LNK_GMAIL."?ui=1&view=up",
				$this->GM_LNK_GMAIL."?ui=1&&search=contacts&ct_id=1&cvm=2&view=ct",
				'post',
				$postdata
			);
			GMailer::parse_gmail_response($this->gmail_data);
	...
	*/
	function addSenderToContact($message_id) {
		if ($this->isConnected()) {			
			...
			set_time_limit(150);
			$c = curl_init();
			curl_setopt($c, CURLOPT_URL, $this->GM_LNK_GMAIL."?ui=1".$query);
	...
	function deleteContact($id) {
		if ($this->isConnected()) {						
			...
			if (is_array($id)) {
				$this->gmail_data = GMailer::execute_curl(
					$this->GM_LNK_GMAIL."?ui=1&view=up",
					$this->GM_LNK_GMAIL."?ui=1&view=cl&search=contacts&pnl=a",
					'post',
					$query
				);
			} else {
	...
	*/
	function editLabel($label, $action, $renamelabel) {
		if ($this->isConnected()) {
			...
			if ($this->domain == "") {
				$refer = $this->GM_LNK_GMAIL_HTTP."?ui=1&view=pr&pnl=l".$this->proxy_defeat();
				$url = $this->GM_LNK_GMAIL_HTTP."?ui=1&view=up";
			} else {
				// GAFYD
				$postdata .= "&search=";
/* 				$refer = $this->GM_LNK_GMAIL_HTTP."?ui=1&ik=".$_SESSION['id_key']."&view=pr&pnl=l&lm=m_prefs".$this->proxy_defeat(); */
				$refer = $this->GM_LNK_GMAIL_HTTP."?ui=1&view=pr&pnl=l&lm=m_prefs".$this->proxy_defeat();
				$url = $this->GM_LNK_GMAIL_A."?ui=1&view=up";
			}

			//Debugger::say($this->GM_LNK_GMAIL_HTTP."?ui=1&ik=".$_SESSION['id_key']."&view=pr&pnl=l".$this->proxy_defeat());
			$this->gmail_data = GMailer::execute_curl(
/* 				$this->GM_LNK_GMAIL_HTTP."?ui=1&ik=".$_SESSION['id_key']."&view=up", */
				$url,
/* 				$this->GM_LNK_GMAIL_HTTP."?ui=1&view=up", */
				$refer,
				'post',
				$postdata,
				"", ""
			...
			GMailer::parse_gmail_response($this->gmail_data);
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."editLabel url: ".print_r($this->GM_LNK_GMAIL_HTTP."?ui=1&view=up",true)); */
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."editLabel refer: ".print_r($refer,true)); */
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."editLabel postdata: ".print_r($postdata,true)); */
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."editLabel: ".$this->gmail_data); */
	...
	* @param bool	$trash
	* @param bool	$applyfilternow
	* @param bool	$test_filter
	* @param bool	$markRead	(9 Nov 2007)
	* @author Neerav
	* @since 25 Jun 2005
	*/
	function editFilter($filter_id, $from, $to, $subject, $has, $hasnot, $hasAttach,
				$archive, $star, $label, $label_name, $forward, $forwardto, $trash, 
				$applyfilternow = false, $test_filter = false, $markRead = false) {
...
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."<br />\n$query\n$refer\n"); */
/* 			 */
/* 			exit; */
/*  */
			$this->gmail_data = GMailer::execute_curl(
				$this->GM_LNK_GMAIL."?ui=1&".$query,
				(($refer !== "") ? $this->GM_LNK_GMAIL."?ui=1".$refer : ""),
				'get'
			);
			GMailer::parse_gmail_response($this->gmail_data);
			
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": ".print_r($this->gmail_data,true)); */
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": ".print_r($this->raw,true)); */
	...
	function webClip($action = "get", $thread_id = 0, $mailbox = "") {
		if ($this->isConnected()) {			
			...
			if ($action == "get") {
				...
				$data['act'] = (($action == "remove") ? "de_" : "ae_").$url; // the URL should NOT be encoded, since POSTing it will single encode it, which is only what is required
				$data['at'] = $this->at_value();
	...
	*/
	function deleteFilter($id) {
		if ($this->isConnected()) {			
			...
			$this->gmail_data = GMailer::execute_curl(
				$this->GM_LNK_GMAIL."?ui=1&ik=&view=up",
				$this->GM_LNK_GMAIL."?ui=1&pnl=f&view=pr".$this->proxy_defeat(),
				'post',
				$query
			);
			GMailer::parse_gmail_response($this->gmail_data);
...
/* 			$post_fields['p_ix_fv']	= 		"true"; */
			...
			$this->gmail_data = GMailer::execute_curl(
				$this->GM_LNK_GMAIL."?ui=1".$post_url,
				$this->GM_LNK_GMAIL."?ui=1&view=pr&pnl=g".$this->proxy_defeat(),
				'post',
				$post_fields
			);
...
/* 				//} */
		...
		if (GM_DEBUG_CURL and isset($fd)) {
			@fclose($fd);
			Debugger::say(basename(__FILE__).": ".__LINE__.": "."Gmail response: ".print_r($gmail_response,true),"debug_curl.txt");
		}
...
/* 		$this->cookie_str = GMailer::get_cookies($data, $this->cookie_str); */
		...
		foreach($new_cookies_array as $name => $cookie) {
			...
			if (isset($all_cookies["$name"])) $all_cookies["$name"] = $cookie['value'];
		}
	...
	*/
	function setMobileSetting($mobile_display) {
		...
		if ($this->isConnected()) {
			// ui=1& not required since we are using Gmail Mobile; Neerav; 1 Nov 2007
...
/* 				'get', "" */
/* 			); */
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."test setMobile A: ".print_r($this->gmail_data,true)); */
/* 			 */
/* 			exit; */


/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."post: ".print_r($get,true)); */
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."post url: ".print_r($url,true)); */
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."post data: ".print_r($get,true)); */
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."post referrer: ".print_r($this->GM_LNK_GMAIL_MOBILE.str_replace("&zx=", "", $this->proxy_defeat("nodash"))."-/?v=cmf",true)); */
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."post cookie: ".print_r($this->cookie_str,true)); */
/* 			Debugger::say(basename(__FILE__).": ".__LINE__.": "."cookie needed: ".print_r("Cookie: PREF=ID=e850c57148d6143b:TM=1160652008:LM=1166333423:GM=1:S=lI-8ULtrRIq0NejV; rememberme=true; __utmz=173272373.1162274748.1.1.utmccn=(direct)|utmcsr=(direct)|utmcmd=(none); __utma=173272373.360229672.1162274748.1173849011.1173893867.8; SID=[deleted]; S=gmail=oSldHvjJpu1SivgKNjIUgw:gmail_yj=vH0eUECIEzF9rE6qWXScTg:gmproxy=jbp9JfHGs7s:gmproxy_yj=6G2Rh5ySmf8:gmproxy_yj_sub=eU4Eyy3kwAo; GMAIL_AT=[deleted]; __utmc=173272373; __utmb=173272373; GXAS=modi.tk=[deleted]; GMAIL_HELP=hosted:1; S=gmail=oV_9aSkAbfIlpsgi_TEXRQ:gmail_yj=vH0eUECIEzF9rE6qWXScTg:gmproxy=jbp9JfHGs7s:gmproxy_yj=6G2Rh5ySmf8:gmproxy_yj_sub=eU4Eyy3kwAo:dasher_cpanel=xmb7gqo9PtE; TZ=-330",true)); */

			// save original cookie
			$orig_cookie = $this->cookie_str;
			...
			// return to original cookie
			$this->cookie_str = $orig_cookie;
	...
	*/
	function changeLanguage($new_lang, $old_lang = "") {
		...
		if ($this->isConnected()) {			
			...
			//$refer .= "&ik=".IKVALUE;
			$refer .= "&view=pr&pnl=g";
			$refer .= "&lm=m_prefs";	// added; Neerav; 21 Jan 2008
			$refer .= $this->proxy_defeat();	 // to fool proxy

			$this->gmail_data = GMailer::execute_curl(
				$this->GM_LNK_GMAIL."?ui=1".$query,
				$this->GM_LNK_GMAIL."?ui=1".$refer,
				'get'
			);
...
/*  					break; */
				case "send_verify":	// send the verification email to the From address
					...
					$url 				= "&view=cf&at=".$this->at_value().$this->proxy_defeat();
					$refer 				= $url;
 					break;
...
/* 			$status = (isset($this->raw["ar"][1])) ? $this->raw["ar"][1] : 0; */
	...
	*/
	function exportContacts($format) {
		...
		$this->gmail_data = $this->gmail_data = GMailer::execute_curl(
            $this->GM_LNK_GMAIL."?ui=1&view=fec",
            $this->GM_LNK_GMAIL."?ui=1&view=sec",
            'post',
            $postdata,
            "noheader"
         );
	...
	*/
	function at_value() {
/* 		Debugger::say(basename(__FILE__).": ".__LINE__.": "."at value: ".str_replace("GMAIL_AT=", "", GMailer::gmail_cookie('GMAIL_AT'))); */
/* 		Debugger::say(basename(__FILE__).": ".__LINE__.": "."all cookies: ".GMailer::gmail_cookie()); */
/* 		return str_replace("GMAIL_AT=", "", GMailer::gmail_cookie('GMAIL_AT')); */
/* 		 */
/*  */
...
/* 				$this->snapshot_error = $matches[1]; */
				...
				Debugger::say(basename(__FILE__).": ".__LINE__.": "."libgmailer: invalid datapack -- not an array, dump RAW HTML:\n".print_r($raw_html,true));
			...
			if ($this->snapshot_error == "") 
				Debugger::say(basename(__FILE__).": ".__LINE__.": "."libgmailer ".__LINE__.": unknown snapshot error:\n".print_r($raw_html,true),"error_log.unknown_snapshot_error");
...
/* 		Debugger::say(basename(__FILE__).": ".__LINE__.": "."final web clip: ".print_r($this->web_clips,true)); */
		...
		if ($type & (GM_STANDARD|GM_LABEL|GM_CONVERSATION|GM_QUERY)) {
			...
			if (isset($raw["ds"])) {
				...
				if (isset($raw["tf"])) {	// a known error, as a status message, from Gmail
					...
					Debugger::say(basename(__FILE__).": ".__LINE__.": "."ds AND tf undefined, dumping raw: ". print_r($raw,true));
				}
				return null;
			}
			...
			if ((isset($raw["ct"][1])) and (count($raw["ct"][1]) > 0)) {
				...
				Debugger::say(basename(__FILE__).": ".__LINE__.": ".'ct[1] not set, raw[ct] dump: '.print_r($raw["ct"],true));
			} 
			...
			if (isset($raw["t"])) {					  
				foreach ($raw["t"] as $t) {
					...
					if (isset($t[13-$less])) {
						...
						$tb["has_voicemail"]= $t[16-$less];		// Added by (Gmail) Neerav; 12 Jun 2007;
					} else {
						$tb["is_chat"]		= 0;			// Added by (Gmail) Neerav; 16 Feb 2006;
						$tb["chat_length"]	= "";			// Added by (Gmail) Neerav; 16 Feb 2006;
						//$tb["??"]		= $t[15-$less];		// Added by (Gmail) Neerav; 16 Feb 2006; what is this?? always 0
						$tb["has_voicemail"]= 0;			// Added by (Gmail) Neerav; 12 Jun 2007;
					}
			...
			if (isset($raw["cs"])) {
				//Debugger::say(basename(__FILE__).": ".__LINE__.": "."cs exists: ".print_r($raw["cs"],true));
				//Debugger::say(basename(__FILE__).": ".__LINE__.": "."cs exists, dumping raw: ".print_r($raw,true));
...
/* 				$less_cs  = (count($raw["cs"]) == 12) ? 2 : 0 ; */
/* 				if (count($raw["cs"]) > 14) Debugger::say(basename(__FILE__).": ".__LINE__.": "."extra cs fields: ".print_r($raw["cs"],true)); */
					...
					for ($i = 0; $i < $mg_count; $i++) {
						if ($raw["mg"][$i][0] == "mb" && $i > 0) {
							...
							if (isset($raw["mg"][$i][2])) {
								...
								Debugger::say(basename(__FILE__).": ".__LINE__.": "."libg101 error, dump raw: ".print_r($raw,true));
...
/* 							$b["sender_email"] 	= str_replace("\"", "", $raw["mg"][$i][8]);		// remove annoying d-quotes in address */
								...
								if (!is_array($raw["mg"][$i][18])) {
									...
									foreach ($raw["mg"][$i][18] as $bb) {
										...
										if (!isset($bb[1])) {
											...
											Debugger::say(basename(__FILE__).": ".__LINE__.": "."undefined attachment info, dumping raw: ".print_r($raw,true));
											Debugger::say(basename(__FILE__).": ".__LINE__.": "."undefined attachment info, dumping raw_html: ".print_r($raw_html,true));
										}
									}
								}
							}
							if ($raw["mg"][$i][0] == "mi") {
								...
								if (is_array($raw["mg"][$i][33+$more])) {
									...
									$b["has_voicemail"] = 1;	// added by Neerav; 11 Jun 2007
									// $raw["mg"][$i][33+$more][0] // from email address					// added by Neerav; 30 Jun 2007
									// $raw["mg"][$i][33+$more][1] // length, in seconds					// added by Neerav; 30 Jun 2007
									// $raw["mg"][$i][33+$more][2] // Gmail URL to voice attachment			// added by Neerav; 30 Jun 2007
									// $raw["mg"][$i][33+$more][3] // Service name (usually Google Talk)	// added by Neerav; 30 Jun 2007
									// $raw["mg"][$i][33+$more][4] // Link to download Google Talk 			// added by Neerav; 30 Jun 2007
								}
								...
								$b["has_voicemail"] = 0;	// 33 // added by Neerav; 11 Jun 2007
		...
		if  ($type & GM_CONTACT) {
			...
			$type = "";
			$c_grp_det = "";
			// 'a' is deprecated.  Does not seem to be used in ANY account anymore; Neerav; 16 Nov 2007
/* 			if (isset($raw["a"])) { */
/* 				Debugger::say(basename(__FILE__).": ".__LINE__.": "."uses 'a' for contacts: ".print_r($raw,true)); */
/* 				$c_array = "a"; */
...
/* 						"addresses"	=> ((isset($a[2])) ? str_replace("\"", "", $a[2]) : "") */
		...
		if ($type & (GM_PREFERENCE)) {			
			...
			if (isset($raw["cta"][1])) {
				...
				Debugger::say(basename(__FILE__).": ".__LINE__.": ".'cta[1] not set, printing cta: '.print_r($raw["cta"],true));
			}
			...
			if (isset($raw["fi"][1])) {
				foreach ($raw["fi"][1] as $fi) {
					...
					$b = array(
						...
						// (boolean) mark as read; Added by Neerav; 9 Nov 2007
						"read" 		=> ((isset($fi[2][7]) and ($fi[2][7] == "true" or $fi[2][7] === true)) ? true : false),
						// (boolean) apply star
						"star" 		=> ((isset($fi[2][8]) and ($fi[2][8] == "true" or $fi[2][8] === true)) ? true : false),
						// (boolean) apply label
	...
	*/
	function web_clip_snapshot($raw, $action = "item") {
		...
		if ($action == 'preview') {
			...
			// must be last!
			if (isset($raw['fl'][2][0][6])) $raw = $raw['fl'][2][0][6];
		}
