...
class membres
{
	...
	private $tbl="membres";
	private $chp = array('id'=>'id_membre', 'identifiant'=>'pseudo', 'pass'=>'pass', 'email'=>'email', 'cle'=>'cle', 'droit'=>'groupe', 'ip'=>'ip', 'activite'=>'activite');
    ...
    public function connexion($login, $pass, $pass_deja_crypter=false) 
	{
		...
		$this->sql->query($upd);
			...
			// Si le membre est admin ou modo, on cherche ses droits précis
			if ($d->droit>=5) {
				$q_droits="SELECT * FROM ".PREFIX."admin_droits WHERE ".$this->chp['id']."=".$d->id;
				$sql_droits=$this->sql->query($q_droits);
				while ($dr=$this->sql->getRowAssoc($sql_droits)) {
					foreach($dr as $droit=>$val) {
						if ($droit!=$this->chp['id']) {
							$_SESSION['sess_droits'][$droit]=$val;
						}
					}
				}
			}
...
public function securite_admin($droits=null) {
    ...
    if (!isset($_SESSION['sess_pseudo']) || $_SESSION['sess_groupe']<GROUPE_ADMIN) {
		$this->design->display("admin/403.tpl"); die();
    } 
   ...
   if ($droits) {  
	    ...
	    foreach($droitsA as  $droit) {
			if ($_SESSION['sess_droits'][$droit]!=1) {
				$this->design->display("admin/403.tpl"); die();
			}
		}
