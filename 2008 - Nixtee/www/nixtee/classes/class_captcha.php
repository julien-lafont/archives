...
class captcha {
	...
	private $borderH = false;
	private $borderD = false;
	private $border = false;
	private $borderG = false;
	private $type = '';
	...
	public function setFont($font, $size) {
		if(!is_readable($font)) {
			throw new Exception('La police est introuvable  --  '.$font);
		}
		$this->font = $font;
		$this->fontSize = $size;
	}
	 ...
	 */
	public function setMarginFromBorder($margin, $d=false, $b=false, $g=false) {
		if (!$d || !$b || !$g) {
	...
	public function getImage() {
		...
		if(!$this->imageHeight) {
			...
			if ($this->fromBorder===false) 
				$this->imageHeight = $boxHeight + $this->borderWidth*2 + $this->borderH + $this->borderB;
			else
		...
		if(!$this->imageWidth) {
			...
			if ($this->fromBorder===false) 
				$this->imageWidth =  $boxWidth + $this->borderWidth*2 + $this->borderG + $this->borderD;
			else
		...
		// centrage vertical
		if ($this->fromBorder===false) 
			$y = $this->imageHeight   - $this->borderWidth - $this->borderH - $this->borderB;
		else
	...
	public function getRandString() {
		if(!$this->randString) {
			$T = array_merge(range('A', 'H') , range('J','N'), range('P','Z'), range(2,9));
			shuffle($T);
