...
function stats_rss($feed, $retour=3) {
	$fb =& new feedburner($feed);
	$info = $fb->getFeedData();

	$entries = $info['entries'][1];
	if ($retour==1) 	return $entries['circulation'];
	elseif ($retour==2)	return $entries['hits'];
	else				return array("lecteurs" => $entries['circulation'],
				 					  "hits"     => $entries['hits']);
