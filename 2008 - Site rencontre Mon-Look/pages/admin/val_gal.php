...
<table style="width:100%; border:0">
	...
	// $sql=mysql_query("SELECT id_membre, img_principale, username FROM members WHERE img_principale!='' AND img_valid='0'");
	 $sql=mysql_query("SELECT id, pseudo, img, dir_galerie FROM photos LEFT JOIN members ON photos.id_membre=members.id_membre WHERE valid='0'");
		while ($d=mysql_fetch_object($sql)) {
	echo "<span id='d$d->id' class='imggg'><img src='upload/galerie/".$d->dir_galerie."/_min_".$d->img."' style='border:1px solid #000000'></span><br>
	      <script>dndMgr.registerDraggable( new CustomDraggable($('d".$d->id."'), '<img src=\"upload/galerie/".$d->dir_galerie."/_min_".$d->img."\">|".ucfirst($d->img)."') );</script>";
}
		echo'</div></td>
	<td id="dropBox" style="height:170px; width:135px; padding:0 10px 0 10px;vertical-align:top; color:#00E426; font-size:12px; font-weight:bold; text-align:center">Accepter
	 ...
	 <td id="dropBox2" style="height:170px; width:135px; padding:0 15px 0 10px;vertical-align:top; color:#FF0000; font-size:12px; font-weight:bold; text-align:center">Refuser
	...
	<tr>
	 ...
	 <div style="border:1px solid #FFFFFF; height:80px; text-align:center; vertical-align:top">
	    <div id="securees" style="display:none">'.$_SESSION['sess_secure'].'</div>
	 	<div class="envoyer" style="width:115px; margin-top:5px; margin-left:auto; margin-right:auto" OnClick="fini_gal()" >Valider</div>
