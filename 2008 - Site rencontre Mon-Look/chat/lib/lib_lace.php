...
function formatFileContents($rawFileContents)
{
	...
	for ($i = 0; $i < $count; $i++)
	{
		...
		if (substr($fields[0], 0, 4) == 'date')
		{
			...
			$date    = ($first) ? substr($fields[1], 1) : $fields[1];

			$output .= '<div' . $class . '><h4>'.$date.'</h4></div>'."\n";
			continue;
		}

		if (substr($fields[0], 0, 4) == 'hour')
		{
			if (LACE_SHOW_HOUR === true)
			{
				...
				$hour    = ($first) ? substr($fields[1], 1) : $fields[1];
		...
		//$output .= '><span class="name">';
			...
			// Perso
			if ($name{0}=="F") $output .= '><span class="namef">';
			else if ($name{0}=="H") $output .= '><span class="nameh">';
			else $output .= '><span class="namea">';
