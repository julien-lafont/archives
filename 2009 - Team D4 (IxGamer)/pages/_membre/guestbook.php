...
$writeMess='
		...
		<div id="writeMessage" style="display:none; text-align:center"><br />
		...
		<div id="smiley">
		  ...
		  <a href="javascript:emoticon(\':!:\')"><img src="images/smileys/sign56.gif" width="20" height="20" border="0" /></a><br>
			...
			<br /><input type="submit" class="submit" value="envoyer" /><br /><br />
		</fieldset></form>
	</div>
';
//----------------------------------------------------------------------------------------------
...
if ($nbG==0)
{
	...
	$design->zone('titre', 'Guestbook de '.ucfirst($_GET['pseudo']).' &nbsp;-&nbsp; <i>D4team.com</i>');
...
else 
{
	...
	while ($d=mysql_fetch_object($sql))
	{
		...
		$titre='#'.$num.' &nbsp;<img src=images/'.$img.' /> <b>'.ucfirst($d->pseudo).'</b> 
				@ <span style="font-size:9px" id="lien_guestbook">'.$date.' &nbsp;&nbsp; 
				[<a href="Guestbook/'.$d->pseudo.'/">guestbook</a>] &nbsp; 
				[<a href="Galerie-photo/'.$d->pseudo.'/">galerie</a>] &nbsp; 
				[<a href="Profil/'.$d->pseudo.'/">profil</a>]</span>';
	...
	$design->blocOccurence( array("Ecrire un message", $writeMess) );
}
