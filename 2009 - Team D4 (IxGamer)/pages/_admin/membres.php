	...
	$c='<div id="retour"><a href="?admin-accueil">&lsaquo; Retour &lsaquo;</a></div>
		...
		else 
		{
			...
			$c.='<center><b>Membre : <u>'.ucfirst($d->pseudo).'</u></b></center><br>
				 <center><a href="?admin-membres&action=suppr&id='.$d->id.'" onclick="return confirm(\'Etes vous sur de vouloir supprimer le membre '.$d->membre.' ?\');">Supprimer le membre</a></center><br>
			
			<form method="post" action="?admin-membres&action=modifier&id='.$id.'">
			<table id="table_ad_membre">
				<tr>
			...
			foreach($champs as $champ=>$valeur) 
			{
				...
				switch($valeur) {
					...
					break;
					case "mdp":
					  $plus='<span id="lienPass"><a href="#" onclick="$(\'coderPass\').style.display=\'inline\'; Element.toggle(\'lienPass\'); return false;">Crypter un mdp</a></span>
							<div id="coderPass" style="display:none">
								Nouveau mdp : <input type="text" id="newPass" style="width:50px" /> <input type="submit" class="submit" value="crypter" style="width:50px" onclick="crypterPass();" />
							</div>';
					break;
					case "groupe":
					  $plus='<b>0</b>:inactif - <b>1</b>:membre - <b>5</b>:admin - <b>9</b>:BANNI';
					break;
				}
