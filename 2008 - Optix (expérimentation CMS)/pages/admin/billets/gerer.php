...
$m->mbre->securite_admin("billets_edition");
	...
	if (!empty($_GET['mess'])) {
		...
		if ($mess=="suppr_erreur") $retour=miseEnForme_admin('bad', "La suppression de l'actu a échouée");	
...
default:
	...
	$fil_ariane='<a href="admin.php?billets-revisions">Gestions des actus</a> / <strong>actus en attente de publication</strong>';
	...
	$sql=mysql_query("	SELECT id_news, titre, date, pseudo, billet_statut
						...
						WHERE billet_statut!='en ligne'
						ORDER BY b.id_news DESC") or die(mysql_error());
						
	
		$c.="<h3>Actus en en attente de publications</h3>
		...
		while($d=mysql_fetch_array($sql)) {
			...
			$c.= '<tr>
					...
					<td><a href="actualite-'.$id_news.'-'.fonctions::recode($titre).'.htm" target="_blank">'.$titre.'</td>
					<td style="font-weight:bold; color:'.$color.'">'.strtoupper($billet_statut).'</td>
					<td>'.$pseudo.'</td>
					<td style="font-size:10px">'.$date.'</td>
					<td><a href="admin.php?billets-revisions&action=editer&id='.$id_news.'"><img src="images/admin/page_edit.png" alt="Editer" /></a>&nbsp;
						<a href="admin.php?billets-revisions&action=supprimer&id='.$id_news.'"><img src="images/admin/page_delete.png" alt="Supprimer" /></a>
						</td>
				</tr>';
...
case "search":
	...
	$fil_ariane.='<a href="admin.php?billets-revisions">Gestions des actus</a> / <strong>Rechercher une actu</strong>';
	$c=$recherche;
	...
	// On effectue la requête
	$sql=mysql_query("	SELECT id_news, titre, date, pseudo
						FROM $table b
	...
	if (mysql_num_rows($sql)==0) $c.="<div class='error'>Aucune actu trouv&eacute;e</div>";	
