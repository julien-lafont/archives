...
function smarty_function_tag_cloud($params, &$smarty)
{
	...
	$tags = $params['tags'];
	$av_url = $params['av_url'];
	$ap_url = $params['ap_url'];
	...
	foreach ($tags as $tag => $count) {
		...
		$cloud_tags[] = '<a style="font-size: '. floor($size) . 'px'  
			. '" class="'.$class.'" href="' . $av_url . strtolower($tag) . $ap_url 
			. '" rel="'.$tag.'" title="Billets liés au tag : ' . strtolower($tag)  . ' (' . $count . ' points)">' 
			. ucfirst(htmlspecialchars(stripslashes($tag))) . '</a>';
	}
	$cloud_html = join("\n", $cloud_tags) . "\n";
	return $cloud_html;
}
