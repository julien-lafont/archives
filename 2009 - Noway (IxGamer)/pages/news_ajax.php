<?php
/**
 * Appel Ajax - Page News
 * Nécessaire à certaines actions du module News
 *
 */
...
case "com_send":
	...
	$sql=mysql_query("INSERT INTO ".PREFIX."news_com
						(`id_news`, `id_auteur`, `message`, `date`, `ip`)
					   VALUES
						('$id', '$id_auteur', '$message', NOW(), '$ip')");
					
	$sql2=mysql_query("UPDATE ".PREFIX."news
					   SET nb_com=nb_com+1
					   WHERE id=$id");
...
case "com_suppr":
	...
	$sql  = mysql_query("DELETE FROM ".PREFIX."news_com WHERE id=$id");
	$sql2 = mysql_query("UPDATE ".PREFIX."news SET nb_com=nb_com-1 WHERE id=$idNews");
...
case "com_edit":
	...
	{
		securite_admin(true);
		$sql=mysql_query("SELECT * FROM ".PREFIX."news_com WHERE id=$id");
	}
	else
	{
		$myId=$_SESSION['sess_id'];
		$sql=mysql_query("SELECT * FROM ".PREFIX."news_com WHERE id='$id' AND id_auteur='$myId'");
	}
...
case "com_edit2":
	...
	{
		securite_admin(true);
		$sql=mysql_query("UPDATE ".PREFIX."news_com SET message='$mess' WHERE id='$id'");
	}
	else
	{
		$myId=$_SESSION['sess_id'];
		$sql=mysql_query("UPDATE ".PREFIX."news_com SET message='$mess' WHERE id='$id' AND id_auteur='$myId'");
	}
