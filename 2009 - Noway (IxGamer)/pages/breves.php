...
/**
 * Module Brèves
 * Affiche les brèves avec gestion des commentaires
 *
 * Url : /breves/#/Description_facultative.htm ( #=numéro de la breve )
 */
	...
	$writeCom='<br />
		...
		<div id="smiley" style="display:none">
		  ...
		  <a href="javascript:emoticon(\':!:\')"><img src="images/smileys/sign56.gif" width="20" height="20" border="0" /></a><br>
			...
			<br /><input type="submit" class="submit" value="envoyer" style="width:120px" id="submitbreve" /><br /><br />
		</fieldset></form>';
//----------------------------------------------------------------------------------------------
	...
	if ($nb==0) {
		...
		// Actions admin
		if (is_admin()) $admin='<div class="bloc_admin_news" id="bloc_admin" style="float:right; filter:alpha(opacity=100);-moz-opacity:1.0;opacity:1.0; margin-right:10px; margin-top:27px"><a href="?admin-breves&action=editer&id='.$d->id.'"><img src="images/boutons/playlist.png" /></a> &nbsp;<a href="#" onclick="ConfirmSupprBreve('.$d->id.'); return false"><img src="images/boutons/del.png" /></a></div>';
	...
	{
		// Mise en couleur alternée
		if ($i%2==0) $class="comDetailA";
		else		 $class="comDetailB";
		if ($i==0)	 $class="comDetailATop";
		
		// On gère l'avatar
		if (empty($b->avatar)) 	$avatar="images/upload/avatar/no_avatar.gif";
		else					$avatar="images/upload/avatar/".$b->avatar;
	...
	$design->zone('titrePage', 'News Scène Esport : '.recupBdd($d->titre));	
	$design->zone('contenu', $contenu);
	metatag('Actualité sur la scène esport : '.recupBdd($d->contenu), extraire_keywords(recupBdd($d->contenu)));
	$design->zone('header', '<script type="text/javascript" src="javascript/-breves.js"></script>
