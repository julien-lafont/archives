...
function getResultValues()
    {
    oTable = document.getElementById("inpList2"+this.oName);
    sArr = ""
    for (var i=0;i<oTable.rows.length;i++)
        {
        if(oTable.rows[i].childNodes[0].innerHTML!="")
            {
            val = oTable.rows[i].childNodes[0].childNodes[0].innerHTML  
            sArr += "\"" + val + "\","
            }
        }
...
function getResultDisplayed()
    ...
    for (var i=0;i<oTable.rows.length;i++)
        ...
        if(oTable.rows[i].childNodes[0].innerText!="")
            {
            display = oTable.rows[i].childNodes[0].childNodes[1].innerHTML
            sArr += "\"" + display + "\","
            }
        }
    sArr = sArr.substring(0,sArr.length-1)
...
function RUN()
    {
    sHTML = "<input type=hidden name=inpObjName id=inpObjName value='"+this.oName+"'>" +
...
function listClick1(e,arrItem)
    ...
    e.style.color = "white"
    oTable = document.getElementById("inpList1"+this.oName);
    for (var i=0;i<oTable.rows.length;i++)
        {
        if(e!=oTable.rows[i].childNodes[0]) 
            {
            oTable.rows[i].childNodes[0].style.backgroundColor = "";
            oTable.rows[i].childNodes[0].style.color = "";
            }
        }
    
    this.row1 = e.parentNode //store selected row
    }
    
function listClick2()
    {
    oName = document.getElementById("inpObjName").value //this.oName //=> fail
    oTD = this;
    if(oTD.tagName=="TR")
        {
        oTD = oTD.childNodes[0]
        }
    oTD.style.backgroundColor = "#316AC5"
    oTD.style.color = "white"
    oTable = document.getElementById("inpList2"+oName);
    for (var i=0;i<oTable.rows.length;i++)
        {
...
function add()
    ...
    var spans = this.row1.childNodes[0].getElementsByTagName("SPAN");
    val = spans[0].innerHTML;
    display = spans[1].innerHTML;
    style = spans[1].style.cssText
    ...
    this.row1.childNodes[0].style.backgroundColor = "";
...
function remove()
    ...
    val2 = this.row2.childNodes[0].childNodes[0].innerHTML
    display2 = this.row2.childNodes[0].childNodes[1].innerHTML
