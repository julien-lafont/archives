...
function smarty_modifier_date_format_pretty($string, $timestamp=false)
{
  ...
  if( $date > strtotime('today 00:00:00') )
    $d = "Aujourd'hui &agrave; ".strftime( '%H:%M', $date );
  elseif( $date > strtotime('yesterday 00:00:00') )
    $d = $langs[$l[0]][0].' &agrave; '.strftime( '%H:%M', $date );
  elseif( $date > strtotime('-2 days 00:00:00') )  
    $d = $langs[$l[0]][1].' &agrave; '.strftime( '%H:%M', $date );
  elseif( $date > strtotime('-1 week 00:00:00') )
    $d = 'Le '.strftime( '%A %d %B', $date).' &agrave; '.strftime('%H:%M', $date );
  else
    $d = 'Le '.strftime( '%d %B %Y', $date).' &agrave; '.strftime('%H:%M', $date );
